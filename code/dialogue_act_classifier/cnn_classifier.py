...
#import a csv file and append the columns to be classified into class, data
  def read_data(self):
    ...
    classify_data = pd.read_csv(filepath, sep = '\t') #change the seperator dependening on your file
  #preprocess the data before converting them to below list of values if required.
    classes = [str(a).lower().strip() for a in classify_data['classes_col'].values]
    data = [str(a).lower().strip() for a in classify_data['data_col'].values]
    yield classes,data

  def transform_data(self):
    class_val, data_val = self.read_data()
    utt_train, utt_test, acts_train, acts_test = train_test_split(data_val,class_val,
                                                                test_size=0.4,
  ...
  def create_embeding(self):
    ...
    for _, wi in enumerate(self.tokenizeX.word_index.items()):
      w, i = wi
      embed_matrix[i, :] = word_emb[w] if w in word_emb else np.random.normal(0,1, (1,embed_dim))
  ...
  def model(self):
    for matrices in self.create_embeding():
      ...
      self.model.add(layers.Embedding(self.vocab_size, embed_dim, weights=[embed_matrix],
