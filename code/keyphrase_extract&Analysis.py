...
class keyphrase_topic_categories():
    ...
    def __init__(self):
        self.cntrdct_wrds = ['yet','nevertheless','nontheless','even so','however',
                        'still','despite that', 'inspite of that','though','although']
        self.non_affrmng_wrds = ['no','no indeed','absolutely not','most certainly not',
                            'ofcourse not','under no circumstance','by no means',
                            'not at all','not really','no thanks', 'nae', 'nope','no way','nah']
        self.clarifying_wrds = ['sorry','did not get', 'did not understand','what','did not follow','come again'
...
if __name__ == '__main__':
    ...
    keyphrase = classy.keyphrase_category(dataHypo, dataHypo['Utterance_text'], dataHypo['dial_keyphrases'])